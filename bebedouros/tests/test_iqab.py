...
class CalcularTests(TestCase):
    ...
    def test_tudo_dentro_dos_limites_calcula_100(self):
        ...
        self.assertEqual(d["iqab"], Decimal("100.0"))
        self.assertEqual(d["classificacao"], "Excelente")
        self.assertEqual(d["qfq"], Decimal("100.0"))
        self.assertEqual(d["qm"], Decimal("100.0"))
        self.assertEqual(d["co"], Decimal("100.0"))
        self.assertEqual(d["versao"], iqab.VERSAO_METODOLOGIA)

    def test_condutividade_nao_entra_na_formula(self):
        r = self._criar(condutividade=Decimal("9999"))
        self.assertEqual(iqab.calcular(r)["iqab"], Decimal("100.0"))
    ...
    def test_cloro_fora_da_faixa_ideal_derruba_qfq(self):
        ...
        self.assertEqual(d["qfq"], Decimal("65.0"))
        self.assertEqual(d["iqab"], Decimal("89.5"))  # 0,3*65 + 0,5*100 + 0,2*100

    def test_turbidez_faixa_intermediaria_pontua_50(self):
        r = self._criar(turbidez_valor=Decimal("3"))  # 1,01 a 5 NTU
        d = iqab.calcular(r)
        # QFQ = 0,35*100 + 0,25*50 + 0,20*100 + 0,20*100 = 87,5
        self.assertEqual(d["qfq"], Decimal("87.5"))
        self.assertEqual(d["iqab"], Decimal("96.3"))  # 96,25 arredondado p/ cima

    def test_turbidez_acima_do_limite_zera(self):
        r = self._criar(turbidez_valor=Decimal("6"))
        d = iqab.calcular(r)
        self.assertEqual(d["qfq"], Decimal("75.0"))
        self.assertEqual(d["iqab"], Decimal("92.5"))
    ...
    def test_ph_fora_da_faixa_ideal_zera(self):
        ...
        d = iqab.calcular(r)
        self.assertEqual(d["qfq"], Decimal("80.0"))
        self.assertEqual(d["iqab"], Decimal("94.0"))
    ...
    def test_ecoli_presente_zera_qm_mesmo_com_coliformes_ausente(self):
        ...
        d = iqab.calcular(r)
        self.assertEqual(d["qm"], Decimal("0.0"))
        self.assertEqual(d["iqab"], Decimal("50.0"))  # 0,3*100 + 0,5*0 + 0,2*100
    ...
    def test_coliformes_presente_sem_ecoli_usa_media_ponderada(self):
        ...
        self.assertEqual(d["qm"], Decimal("70.0"))
        self.assertEqual(d["iqab"], Decimal("85.0"))  # 0,3*100 + 0,5*70 + 0,2*100

    def test_filtro_vencido_zera_co(self):
        r = self._criar(filtro=Resultado.FILTRO_VENCIDO)
        d = iqab.calcular(r)
        self.assertEqual(d["co"], Decimal("0.0"))
        self.assertEqual(d["iqab"], Decimal("80.0"))  # 0,3*100 + 0,5*100 + 0,2*0
        self.assertEqual(d["classificacao"], "Excelente")
...
class RecalculoAoSalvarTests(TestCase):
    ...
    def test_salvar_grade_completa_calcula_iqab(self):
        ...
        self.assertEqual(r.iqab, Decimal("100.0"))
        self.assertEqual(r.iqab_classificacao, "Excelente")
        self.assertEqual(r.iqab_texto(), "100.0 · Excelente")
